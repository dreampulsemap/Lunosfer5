...
class ProfileRepository {
    ...
    suspend fun uploadAvatar(byteArray: ByteArray, fileName: String): Result<String> = runCatching {
        ...
        // "avatars" bucket'ının RLS INSERT politikası, yüklenen objenin
        // yolundaki ilk klasör segmentinin yükleyenin auth.uid()'sine eşit
        // olmasını şart koşuyor (diğer bucket'larla aynı desen: diary-media,
        // goal-images, message-attachments, vb.). Düz bir dosya adına
        // (klasörsüz) yüklemek bu kontrolü hiçbir zaman sağlamıyor ve
        // "new row violates row-level security policy" ile başarısız
        // oluyordu.
