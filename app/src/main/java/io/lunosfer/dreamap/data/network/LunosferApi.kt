...
interface LunosferApi {
    ...
    suspend fun likeDiaryEntry(
        @Body input: io.lunosfer.dreamap.data.model.DiaryLikeRequest
    ): io.lunosfer.dreamap.data.model.DiaryLikeResponse

    @HTTP(method = "DELETE", path = "api/diary/like", hasBody = true)
    suspend fun unlikeDiaryEntry(
        @Body input: io.lunosfer.dreamap.data.model.DiaryLikeRequest
    ): io.lunosfer.dreamap.data.model.DiaryLikeResponse

    @GET("api/diary/comment")
    suspend fun getDiaryComments(
        @Query("diaryEntryId") diaryEntryId: String
    ): io.lunosfer.dreamap.data.model.DiaryCommentsResponse

    @POST("api/diary/comment")
    suspend fun createDiaryComment(
        @Body input: io.lunosfer.dreamap.data.model.CreateDiaryCommentRequest
    ): io.lunosfer.dreamap.data.model.CreateDiaryCommentResponse

    @HTTP(method = "DELETE", path = "api/diary/comment", hasBody = true)
    suspend fun deleteDiaryComment(
        @Body input: io.lunosfer.dreamap.data.model.DeleteDiaryCommentRequest
    ): io.lunosfer.dreamap.data.model.GenericSuccessResponse
